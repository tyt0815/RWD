...
- 에디터에서 텍스트 입력은 `editor/ui/TextInput.lua`, 선택·검색 목록은 `editor/ui/ComboBox.lua`를 생성·조합해 사용한다. 화면별 코드에서 커서 이동, 삽입·삭제, 필터링, 드롭다운 상태를 다시 구현하지 않는다.
- Lua UI는 구현 상속보다 공통 모듈 인스턴스의 조합을 우선한다. 새 입력 UI가 필요하면 기존 공통 모듈을 확장하거나 새 단일 책임 컴포넌트를 `editor/ui/`에 추가한다.
- RadioButton 등 요청되지 않은 UI 컴포넌트는 미리 구현하지 않는다.
- Project에서 공통 UI가 필요해지면 `projects/`가 `editor/`를 불러오지 않도록 중립적인 공용 UI 경계를 먼저 설계한다.
