...
`MetronomePlayback`은 한 BPM beat 길이의 Editor 전용 정적 Source를 만든다. Period 4에서는 첫 subdivision에 1760Hz 강박, 나머지 세 subdivision에 880Hz 일반박을 만들고 loop와 pitch로 재생 위치·속도를 맞춘다. Core와 Project는 이 Editor 디버깅 음원을 알지 못한다.
