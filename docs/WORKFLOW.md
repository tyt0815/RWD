...
`F`는 Play와 Pause를 전환하고 `Ctrl+S`는 현재 Stage를 저장한다. `Ctrl+Z`는 노드 배치·이동·삭제·프로퍼티 변경과 Stage 설정 편집을 되돌리고 `Ctrl+Shift+Z`는 다시 실행한다. Undo 뒤 새 편집을 시작하면 기존 redo 이력은 제거된다. `R`은 재생 중이면 먼저 Pause한 뒤 기준 바와 Timeline 시작 위치를 beat 0으로 되돌린다. Dialog나 숫자 Value를 편집 중일 때는 이 단축키를 실행하지 않으며 key repeat도 무시한다.
